...
print("\n" + "="*70)
print("INTEGRATION TEST: SIGNUP -> LOGIN -> CHAT HISTORY")
print("="*70 + "\n")
...
if resp.status_code == 302:
    print("OK - Signup redirected (302)")
else:
    print(f"FAIL - Signup status: {resp.status_code}")

user1 = User.objects.filter(username='test_user1').first()
if user1:
    print(f"OK - User created: {user1.username} ({user1.email})")
    try:
        profile = user1.profile
        print(f"OK - User profile found: company={profile.company}, role={profile.role}")
    except:
        print("FAIL - User profile not found")
else:
    print("FAIL - User not created in database")
...
print(f"Status: {resp.status_code}")
if resp.status_code == 302:
    print("OK - Login redirected (302)")
else:
    print(f"FAIL - Login status: {resp.status_code}")

# Check if user is authenticated in session
if '_auth_user_id' in client.session:
    print(f"OK - User authenticated in session (ID: {client.session.get('_auth_user_id')})")
else:
    print("FAIL - User not authenticated in session")
...
if user1:
    ...
    print(f"OK - Created conversation 1: {conv1.title}")
    
    conv2 = Conversation.objects.create(
        user=user1,
        title="Conversation 2 - Gas Production"
    )
    print(f"OK - Created conversation 2: {conv2.title}")
...
if user1:
    ...
    if resp_me.status_code == 200:
        ...
        print(f"OK - Current user ID from API: {user_id}")
        ...
        if resp_history.status_code == 200:
            ...
            print(f"OK - Chat history API returned {count} conversations")
            
            if count >= 2:
                print("OK - Both conversations found in history")
                for conv in history_data.get('history', []):
                    print(f"  - {conv.get('title')}")
            else:
                print(f"FAIL - Expected 2 conversations, got {count}")
        else:
            print(f"FAIL - Chat history API status: {resp_history.status_code}")
    else:
        print(f"FAIL - /api/v1/users/me/ status: {resp_me.status_code}")
...
if user2:
    ...
    print(f"OK - Created conversation for user2: {conv3.title}")
    ...
    print(f"\nOK - User1 has {user1_conversations} conversations")
    print(f"OK - User2 has {user2_conversations} conversations")
    
    if user1_conversations == 2 and user2_conversations == 1:
        print("OK - Users have correct conversation counts (isolated)")
    else:
        print("FAIL - Conversation counts do not match expected")
else:
    print("FAIL - Second user not created")
